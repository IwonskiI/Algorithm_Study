		...
		for(int b = 0; b < 2; b++) {
			...
			for(; c < N; c++) {
				...
				// 현재 높이가 다음 높이보다 1 높다면
					...
					// 반복문 종료 후 지나갈 수 없었다면 종료
					if(!flag) break;
					// 아니라면
					else {
						// 현재 높이는 낮아진 높이로 설정
						cur = road[c];
						// 현재 칸에는 경사로를 놓은 마지막 칸이 될 것이므로 새로 경사로를 겹칠 수 없기 때문에 길이는 0
						// (다음 칸부터 경사로를 놓을 수 있는 칸으로 증가)
						len = 0;
					}
				}
				// 높이가 같거나 1차이가 아니라면 경사로를 놔도 지나갈 수 없으므로 종료
				else {
					flag = false;
					break;
				}
			}
			// 일직선 테스트를 완료했을 때 지나갈 수 있다면 정답 증가
			if(flag) ans++;
		}
