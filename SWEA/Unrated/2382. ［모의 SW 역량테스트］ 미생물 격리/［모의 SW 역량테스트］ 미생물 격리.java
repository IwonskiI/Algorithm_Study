...
public class Solution {
	...
	public static int[][][] board, nboard;
	
	public static void move(int r, int c) {
		// 현재 미생물 정보 가져오기
		int cnt = board[r][c][0];
		int dd = board[r][c][1];
		int prev = board[r][c][2];
		// 새로운 좌표
		...
		if(nboard[nr][nc][0] == 0) {
			...
			if(cnt != 0) nboard[nr][nc] = new int[] {cnt, dd, prev};
		}
		// 이동하려는 칸에 미생물이 있다면
		else {
			// 현재 미생물의 수가 해당 칸에 합쳐진 미생물 수의 최대치보다 많다면 이동 방향을 현재 미생물 방향으로 전환
			if(cnt > nboard[nr][nc][2]) {
				nboard[nr][nc][2] = cnt;
				nboard[nr][nc][1] = dd;
			}
...
//            	Virus cur = new Virus(cnt, dirmap.get(Integer.parseInt(st.nextToken())), cnt);
            	// 해당 좌표에 미생물 저장
            	board[r][c] = new int[] {cnt, dirmap.get(Integer.parseInt(st.nextToken())), cnt};
            }
            
            // M시간동안 진행
            for(int i = 1; i <= M; i++) {
            	nboard = new int[N][N][3];
            	for(int r = 0; r < N; r++) {
            		for(int c = 0; c < N; c++) {
